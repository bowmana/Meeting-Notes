...
### sherpa-onnx diarization over-segments speakers with default threshold (0.7)
**Date:** 2026-03-01
**Context:** A 2-person meeting conversation was consistently detected as 4-5 speakers. The sherpa-onnx spectral clustering threshold was set to 0.7f (where higher = more merging = fewer speakers). Natural voice variation within a single speaker (emotion, volume, speaking style) produces embedding similarities of 0.65-0.75, which fell below the threshold, causing same-speaker segments to be split into multiple clusters.
**What went wrong:** Three compounding issues: (1) The clustering threshold of 0.7 was too low — same-speaker voice variations scored below it, splitting one person into 2-3 "speakers". (2) `MinDurationOff` at 0.5s was too short — natural pauses within a speaker's turn created false speaker boundaries. (3) No post-processing pass existed to merge over-segmented speakers whose embeddings were clearly similar.
**Fix / Lesson:** Increased default threshold from 0.7 to 0.85, increased MinDurationOff from 0.5 to 0.8, and added a post-diarization embedding-based speaker merge step (`MergeSimilarSpeakersAsync`) that extracts per-speaker voice embeddings using the existing CampPlus model, computes pairwise cosine similarity, and merges speakers above a 0.75 similarity threshold. Also fixed speaker count calculation from `Max(index)+1` to `Distinct().Count()` since speaker indices can have gaps after merging. **When using clustering-based diarization, always validate default thresholds with real conversations — academic defaults are often tuned for benchmarks, not real-world small-meeting scenarios. Add post-processing merge passes as a safety net.**
