...
Due to some break changes in llama.cpp, making a break change will help to maintain the good abstraction and friendly user APIs.
A very improtant feature cannot be implemented unless refactoring some parts.
After some discussions, an agreement was reached that making the break change is reasonable.
...
Add the example to LLama.Examples of the repository.
Put the example in another repositpry and add the link to the readme or docs of LLamaSharp.
Add documents
